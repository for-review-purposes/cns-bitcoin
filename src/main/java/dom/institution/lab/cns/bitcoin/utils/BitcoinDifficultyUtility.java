...
public class BitcoinDifficultyUtility {
	...
	// equivalent CNS difficulty for the initial bitcoin target (search space/success space)
	public static final double CNS_INITIAL_DIFFICULTY = 1.0/INITIAL_TARGET.divide(MAX_HASH_VALUE, rounding).doubleValue();

	// six conversions: difficulty <-> CNS, bdiff <-> CNS, pdiff <-> CNS

	/**
	 * Converts from Bitcoin difficulty to CNS difficulty.
	 ...
	 * @return The equivalent CNS difficulty.
	 */
	public static double BTCToCNS(double BTCDiff) {
		return CNS_INITIAL_DIFFICULTY * BTCDiff;
	}

	/**
	 * Converts from CNS difficulty to Bitcoin difficulty.
	 * Note that we simply scale the converted initial CNS to BTC difficulty
	 * for the current CNS difficulty level.
	 *
	 * WARNING: may potentially result in impossible Bitcoin difficulty levels
	 * if the CNS difficulty is too low. Be aware that returned BTC difficulties
	 * may be less than 1!
	 *
	 * @param CNSDiff The given CNS difficulty.
	 * @return The equivalent Bitcoin difficulty.
	 */
	public static double CNSToBTC(double CNSDiff) {
		return CNSDiff/CNS_INITIAL_DIFFICULTY;
	}
	 ...
	 * packed target value to the equivalent CNS difficulty directly.
	 *
	 * @param bits The bitcoin metric representing the truncated current target.
	 * @return The equivalent CNS difficulty.
	 */
	public static double packedTargetToCNS(String bits) {
		return BTCToCNS(packedTargetToBTCDifficulty(bits).doubleValue());
	}

	/**
	 * Convenience function to directly convert a CNS difficulty to the
	 * equivalent packed bitcoin target value (bdiff).
	 *
	 * @param CNSDiff The given CNS difficulty level.
	 * @return The equivalent bitcoin packed target value.
	 */
	public static String CNSToPackedTarget(double CNSDiff) {
		return BTCDifficultyToPackedTarget(new BigDecimal(CNSToBTC(CNSDiff)));
	}
	 ...
	 * to the equivalent CNS difficulty directly.
	 *
	 * @param poolTarget The bitcoin metric representing the full current target.
	 * @return The equivalent CNS difficulty.
	 */
	public static double poolTargetToCNS(String poolTarget) {
		return BTCToCNS(poolTargetToBTCDifficulty(poolTarget).doubleValue());
	}
	 ...
	 */
	public static String CNSToPoolTarget(double CNSDiff) {
		return BTCDifficultyToPoolTarget(new BigDecimal(CNSToBTC(CNSDiff)));
	}
